...
class RSVPResponse(db.Model):
    ...
    principal_entree = db.Column(db.String(255), nullable=True)
    principal_plat = db.Column(db.String(255), nullable=True)

    # Partenaire (null si l'invité n'a pas de partenaire)
    partner_attending = db.Column(db.Boolean, nullable=True)
    partner_entree = db.Column(db.String(255), nullable=True)
    partner_plat = db.Column(db.String(255), nullable=True)

    # Enfants — nombre uniquement, pas de noms
    children_attending_count = db.Column(db.Integer, default=0, nullable=False)
    children_ages = db.Column(db.String(300), nullable=True)
    ...
    def to_dict(self):
        return {
            ...
            "principal_entree": self.principal_entree,
            "principal_plat": self.principal_plat,
            "partner_attending": self.partner_attending,
            "partner_entree": self.partner_entree,
            "partner_plat": self.partner_plat,
